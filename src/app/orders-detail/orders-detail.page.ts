...
export class OrdersDetailPage implements OnInit {
      ...
      this.storage.getObject('order').then((data: any) => {
        if(data != null)
        {
          ...
          for (let item of this.order.order_item)
          {
            ...
            printItemOrder += this.leftRight(item.product.name+" ",item.qty)+" \n";
  ...
  printReceipt()
  {
    ...
    // const resultQr = this.content;
     ...
     this.connectToBluetoothPrinter(1)
     .subscribe(_=>{
  ...
  printOrderHeader()
  {
    ...
    const resultByte = result.encode();
     this.connectToBluetoothPrinter(2)
     .subscribe(_=>{
        this.bluetoothSerial.write(resultByte)
        .then(_=>{
         console.log(_);
      ...
      inputs: [
        {
          ...
          value: this.order.customer.phone
        }
      ],
      buttons: [
        {
